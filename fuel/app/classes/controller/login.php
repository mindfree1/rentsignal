...
class Controller_Login extends Controller_Template
{
    public function action_oauth($provider = null)
    {
        ...
        //class_alias('Fuel\\Core\\Messages', 'Messages');
        if ($provider === null)
        {
...
public function action_register()
{
	...
	/*public function action_login()
	{
		...
		if(\Auth::check())
		{
			\Response::redirect_back('index.php');
		}
		...
		if(\Input::method() == 'POST')
		{
			if(\Auth::instance()->login(\Input::param('username'). \INput::param('password')))
			{
				...
				{
					//dont create the remember me cookie
					\Auth::dont_remember_me();
				}

				//Login go back to the page user came fromt or redirect to dshboard later
				\Response::redirect_back('index.php');
			}
		}
	}*/
	...
	/*public function action_oauth($provider = null)
	{
		...
		if($authentication = \Session::get('auth-strategy.authentication', array()))
		{
			...
			$insert_id = $opauth->link_provider(array(
				'parent_id' => $userid,
				'provider' => $authentication['access_token'],
		...
		{
			//no data available, perhaps didnt recieve an OAuth callback previously?
		}
	}*/
}
?>